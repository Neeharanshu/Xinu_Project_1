...
shellcmd xsh_ps(int nargs, char *args[])
{
	...
	printf("%3s %-16s %5s %4s %4s %10s %-10s %10s %27s\n",
		   ...
		   "Stack Ptr", "Stack Size", "Time Elapsed since creation");
	...
	for (i = 0; i < NPROC; i++) {
		...
		printf("%3d %-16s %s %4d %4d 0x%08X 0x%08X %8d %8d\n",
			...
			prptr->prstkptr, prptr->prstklen, (ps_clktime - prptr->clk_create_time_s)*1000 + (ps_ctr1000- prptr->clk_create_time_ms));
	}
	return 0;
}
